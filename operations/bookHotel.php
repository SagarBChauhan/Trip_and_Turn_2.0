...
if(isset($_POST['btn_sub']))
{
    ...
    $query_Insert="INSERT INTO `tbl_payment`(`Amount`, `Payment_Method`, `Account_No`, `Bank_Name`, `IFSC_Code`, `Owner_AC_No`, `Owner_Bank_Name`, `Owner_IFSC_Code`, `Last_Update_Time`) "
            . "VALUES ('$Total','$pymode','$ACNO','$Bank','$IFSC','100002228291','ICICICI','UCSA22',NOW());";
    $Insert=  mysqli_query($con, $query_Insert);
    if($Insert)
    {
        $paymentid=$con->insert_id;
        $sql_update="INSERT INTO `tbl_hotelbooking`(`Hotal_Id`, `Customer_Id`, `Payment_Id`, `Last_Update_Time`) VALUES ($Hotel_Id,$uid,$paymentid,NOW())";
        ...
        if($query_update)
        {
            ...
            $subject="Your Hotel $Name booked successfully";
            $message="<center>
