...
{echo '<div class="row">';
    while($row = mysqli_fetch_array($result)) {   
        echo'   <div class="col-md-3 ml-auto mr-auto">
                ...
                $s = substr($row['Description'], 0, 75);
        $resul = substr($s, 0, strrpos($s, ' '));
        $resul = substr($s, 0, strrpos($s, ' '));
        if(strlen($row['Description'])>75){echo $resul.'...';}else{echo $row['Description'];} 
                                   echo'  </p>
                        </div>
        ...
        if($vp==1)
        {
            echo '  <a href="Update_Package.php?fetch=' . $row['Package_Id'] .'" target="_blank" class="btn btn-success btn-just-icon btn-fill btn-round btn-wd">
